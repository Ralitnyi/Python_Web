...
async def get_contact(id_: int, db: Session) -> Contact:
...
async def update_contact(id_: int, body: ContactBase, db: Session) -> Contact:
    contact = db.query(Contact).filter_by(id=id_).first()
    if contact:
...
async def search_contact(
    db: Session, name: str = None, last_name: str = None, email: str = None
):
...
async def birthday_per_week(db: Session) -> List[Contact]:
    response = []
